...
def procesar_ticker_safe(ticker, option_types, dias_range, max_expirations, atm_window_range, include_earnings):
    try:
        return procesar_ticker(ticker, option_types, dias_range, max_expirations, atm_window_range, include_earnings)
    except Exception:
        # Si algo raro pasa en un ticker, seguimos con los demás
        return []

def cargar_base(tickers, option_types, dias_range, max_expirations, atm_window_range, include_earnings=False, max_workers=20):
    all_regs = []
